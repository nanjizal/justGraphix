...
import justGraphix.geom.Matrix3x3;
...
abstract Quaternion( Mat1x4 ) from Mat1x4 to Mat1x4 {
    public inline
    function new( m: Mat1x4 ){ this = m; }
    @:from
    public static inline
    function from_xyzw( v: {x: Float, y: Float, z: Float, w: Float} ): Quaternion{
        final m1x4:Mat1x4 = { x: v.x, y: v.y, z: v.z, w: v.w };
        return new Quaternion( m1x4 );
    }
    /**
     * <pre><code>
    ...
    */
    ...
    function lookAt(  target:  Matrix1x4
        ...
        var axis = n1;
        axis.cross( n2 );
        var pointToTarget: Quaternion = ({ x: axis.x, y: axis.y, z: axis.z, w: 1.0 + d }:Quaternion); 
