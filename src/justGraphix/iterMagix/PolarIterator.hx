...
import justGraphix.iterMagix.UnitIterator;

@:access( justGraphix.iterMagix.UnitIterator.count )
@:access( justGraphix.iterMagix.UnitIter.count )
@:access( justGraphix.iterMagix.TauIterator.count )
@:access( IntIterator )
class PolarIter{
...
@:access( justGraphix.iterMagix.PolarIter.count )
@:access( justGraphix.iterMagix.UnitIter.step )
@:forward
abstract PolarIterator ( PolarIter ) from PolarIter to PolarIter {
    public inline
    function new( stepRadius: Float, steps: Int ){
        var radius = new UnitIterator();
        radius.setFloatStep( stepRadius );
