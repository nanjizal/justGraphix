...
import justGraphix.color.space.OKLAB;
import justGraphix.color.space.OKLCH;
import justGraphix.color.space.LAB;
import justGraphix.color.space.SRLAB2;
...
function from_srlch2( L: Float, c: Float, h: Float, alpha: Float ): Int {
   final srlab2: SRLAB2 = new SRLAB2( L, c * Math.cos(h), c * Math.sin(h), alpha  );
   final v: Int = from_srlab2( srlab2 );
   return v;
}
inline
function to_srlch2( v: Int ): SRLCH2 {
   var srlab2 = to_srlab2( v );
   return new SRLCH2( srlab2.L
	  , Math.sqrt( srlab2.a*srlab2.a + srlab2.b*srlab2.b )
	  , Math.atan2( srlab2.b, srlab2.a )
	  , srlab2.alpha );
}
inline
