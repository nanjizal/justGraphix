...
// Test SVG Path's
    ...
    function arcSVG( pen2D: Pen2D ){
        ...
        for( i in 0...yPos.length ){
            drawSet( pen2D, arcs.shift(), pallet, x0, yPos[i], 0.5 );
            drawSet( pen2D, arcs.shift(), pallet, x1, yPos[i], 0.5 );
        }
    }
    // draws a set of svg ellipses.
    inline
    function drawSet( pen2D: Pen2D, arcs: Array<String>, col:Array<Int>, x: Float, y: Float, s: Float ){    
        for( i in 0...arcs.length ) draw_d( pen2D, arcs[ i ], x, y, s, 1., col[ i ] );
    }
    // draws an svg ellipse
    inline
    function draw_d( pen2D: Pen2D, d: String, x: Float, y: Float, s: Float, w: Float, color: Int ){
