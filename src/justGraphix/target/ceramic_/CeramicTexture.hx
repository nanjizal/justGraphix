package justGraphix.target.ceramic_;
// UNTESTED!!!
//#if !'doc-gen'
#if( ceramic )
...
import justGraphix.target.ceramic_.CeramicTexture;

@:forward
@:transient
abstract CeramicImage( Pixelimage ) from Pixelimage to Pixelimage {
    inline
    ...
    @:to
    public inline
    function to(): ceramic.Texture {
        return CeramicTexture.toCeramicTexture_( this );
    }
    public static inline
    function createPixelImage( texture: Texture ): Pixelimage {
        return CeramicTexture.fromCeramicTexture_( texture );
    }
}
